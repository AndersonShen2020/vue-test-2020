// 將滑鼠畫到SVG上，會找出現在的地點並上傳到weather.vue
<script>
import { onMounted, ref, computed, watch, reactive, onUnmounted } from "vue";
import $ from "jquery";

// 用jQuery抓SVG地圖上的城市區塊的城市名稱，整串放到pathInfo中
//   let pathName = $("path").mouseenter(function(e) {
...
export default {
  emits: {
    weather: (location) => {
      console.log("[SvgMap -> emit] 要傳遞的locationName為 = ", location);
      console.log("[SvgMap -> emit] 在SvgMap中emit地點資料出去到上一層");
      return location;
    },
  },
  setup(props, { emit }) {
    ...
    // const pathInfo = (e) => {
    //   let pathName = $("path").mouseenter(function(e) {
    //     tagName.value = $(this).attr("name");
    //     console.log("name = ", tagName.value);
    //   });
    // };

    onMounted(() => {
      // emit("weather", tagName.value);
    });
    ...
    watch(tagName, (newName, oldName) => {
      // console.log("old Area Name = ", oldName);
      // console.log("new Area Name = ", newName);

      console.log(
        `==========================================================\n[SvgMap -> watch] 在SvgMap中使用watch檢測資料更新，並通知emit開始傳遞地點資料出去`
      );
      emit("weather", newName);
    });

    return { demoInfo, svginfo, pathInfo, tagName };
  },
};
</script>
